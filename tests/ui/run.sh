...
# sway.conf), a fresh shell with tests/ui/config as XDG_CONFIG_HOME and
# tests/ui/data as XDG_DATA_HOME on top of the system data dirs, and
# writes to target/ui/<scenario>/: shell.log, sway.log, the screenshots
# it takes, and `status` (ok, or what failed). Input goes through
# `aria-inject` (tests/ui/inject: a virtual keyboard and pointer);
# positions come from the shell's `debug` commands. See lib.sh for the
# vocabulary.
set -u
...
for scenario in $scenarios; do
    ...
    timeout 120 sway -c "$here/sway.conf" > "$out/sway.log" 2>&1
